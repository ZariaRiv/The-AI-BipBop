...
// defining the function to create an instance of the class object
	...
	//{
		...
		// if (bally==40 AND (ballx>=(plankx-10) AND ballx<=(plankx+110)) -> ARGUMETN A
	// int a= speedX;
	// int b= speedY;    // = 5;
	// int c= sqrt(a^2 +b^2);
	// double sinalpha = a/c;
	// double alfa =asin (sinalpha) * 180.0 / PI;
	// double beta = 90-alfa;
	//double degrees=beta;
	// if (ballx >= planxx-10) AND (ballx <= plankx+14){
	//degrees-=30;
	//}
	// else if (ballx >= planxx+86) AND (ballx <= plankx+110){
	//degrees+=30;
	//}
	// how to set new speed? speedY =5 always,
	//speedX = speedY/(tan(degrees))
	// let the ball move
	//else if not ARGUMETN A - GAME OVER, ball fall below the plank or ball is up in the air, so we have to check:
	// if bally<40{ game over
	//else if bally >40 - is in the air
	//}
	...
	//defining a function that prevents the ball from going over the borders of the window
	void borders_bounce(){
		// statement that prevents the ball from going over the right or the left of the screen
		if (this->getX() >=620 || this->getX() <=0 ){
			this->speedX=-this->speedX;
			this->move(0,this->getY());
		}
		// statement that prevents the ball from going over the top or the bottom of the screen
		if (this->getY() <=60 || this->getY() >=460 ){
					this->speedY=-this->speedY;
					this->move(460,this->getY());
				}
	}
...
// function used to start the ball movement
