...
int Platform::handle(int event){
    ...
    case FL_KEYUP:              // keyboard key released
        ...
        if(Fl::event_key() == FL_Left || Fl::event_key() == LKey){
        	//if we are touching the left border, not allowed to move that direction anymore
        	if (currentx <= 0){
                return 1;
                break;
        	}
				...
				this->move((currentx-15), currenty);
        ...
        // if right arrow or D were pressed
        	...
        	//if we are touching the right border, don't allow to move right anymore
        	if (currentx >= 520){
        		return 1;
        		break;
        	}
...
Platform::Platform(int x, int y): Fl_Box(x,y,120,20){
